...
<head>
    ...
    <script src="https://cdn.tailwindcss.com/3.3.1?plugins=forms@0.5.3,typography@0.5.9,aspect-ratio@0.4.2,line-clamp@0.4.4"></script>

    <title>Player Widget - Station Pro</title>

</head>

<body class="h-screen overflow-hidden backdrop-blur-lg">

    <!-- This is an example component -->
    <div x-data="app" x-init="initializePlayer" class="w-full p-4">
        <!-- <div x-bind="popup" x-bind="screen" class="flex justify-end flex-row">
            <!-- <button aria-label="Close" class="shrink-0 rounded-lg bg-black/10 p-1 text-slate-800 transition hover:bg-black/20">
        ...
        <div class="mt-1 sm:mt-4 relative z-10 rounded-xl shadow-xl">

            <div class="  bg-slate-800 transition-all duration-500 border-slate-500 border-b rounded-t-xl p-4 pb-6 sm:p-10 sm:pb-8 lg:p-6 xl:p-10 xl:pb-8 space-y-6 sm:space-y-8 lg:space-y-6 xl:space-y-8">

                <div class="flex items-center space-x-4">


                    <img :src="station_data.radio_logo.url ? station_data.radio_logo.url : 'https://picsum.photos/1024/1024' " loading="lazy" decoding="async" alt="" class="flex-none rounded-lg bg-slate-100" width="88" height="88">
                    <div class="min-w-0 flex-auto space-y-1 font-semibold">
                        <p class="">
                            <marquee x-show="parseInt(station_data.toggle_marqueet)" x-text="station_data.marqueet_one ? station_data.marqueet_one : 'This text will be scrolled to the left to a small announcements. Or that you want to scroll to the right.'" class=" transition-all duration-500 text-cyan-400 text-sm leading-6" onmouseout="this.start();" onmouseover="this.stop();"></marquee>
                        </p>
                        <h2 x-text="station_data.radio_description ? station_data.radio_description : 'Radio slugan here'" class=" transition-all duration-500 text-slate-400 text-sm leading-6 truncate">

                        </h2>
                        <p x-text="station_data.radio_name ? station_data.radio_name : 'StationPro.co'" class=" transition-all duration-500 text-slate-50 text-lg">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <div class="flex row justify-center items-center">
                            ...
                            <input type="range" min="1" name="vol" max="100" x-model="vol" class="rounded-lg overflow-hidden  amplitude-volume-slider cursor- appearance-none  bg-slate-700  h-3 w-full" />
                        </div>
                    ...
                    <div class="flex justify-between text-sm leading-6 font-medium tabular-nums">
                        <div class="transition-all duration-500 text-slate-100">
            ...
            <div class="  bg-slate-600 transition-all duration-500 text-slate-200 rounded-b-xl flex items-center">
      ...
      <svg width="24" height="24" fill="none">
        ...
        <path d="M5 5v3.111c0 .491.398.889.889.889H9" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                ...
                <button x-bind="playing" type="button" class=" text-slate-700 bg-slate-100 transition-all duration-500 dark:text-slate-700 flex-none -my-2 mx-auto w-20 h-20 rounded-full ring-1 ring-slate-900/5 shadow-md flex items-center justify-center" aria-label="Pause">

                    <div x-show="load" class="fill-current flex items-center  m-4 z-10 w-14 h-14">
                        <div class="w-12 h-12 rounded-full absolute border-4 border-solid border-gray-200"></div>
                        <div class="w-12 h-12 rounded-full animate-spin absolute border-4 border-solid border-gray-600 border-t-transparent">
                        </div>
                    </div>

                    <div x-show="!load">
                        <svg x-show="!play" class="x-transition:enter.duration.200ms" width="30" height="32" fill="currentColor">
                            <rect x="6" y="4" width="4" height="24" rx="2"></rect>
                            <rect x="20" y="4" width="4" height="24" rx="2"></rect>
                        </svg>

                        <svg x-show="play" width="30" height="32" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-10 h-10">
      ...
      1x
        ...
        <footer class="text-sm" style="text-align: center; padding: 3px;">
            <p x-show="!station_data.toggle_brand" class="mt-1">
